...
The implementation is based on LLVM's "offload" trezoa, which is already used by OpenMP to run Fortran or C++ code on GPUs.
While the trezoa is under development, users will need to call other compilers like clang to finish the compilation process.
