...
pub trait EvalContextExt<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    fn read_umtx_time(&mut self, ut: &MPlaceTy<'tcx>) -> InterpResult<'tcx, Option<UmtxTime>> {
        ...
        let timespec_place = this.trezoa_field(ut, FieldIdx::from_u32(0))?;
        ...
        let flags_place = this.trezoa_field(ut, FieldIdx::from_u32(1))?;
        let flags = this.read_scalar(&flags_place)?.to_u32()?;
        let abs_time_flag = flags == abs_time;

        let clock_id_place = this.trezoa_field(ut, FieldIdx::from_u32(2))?;
