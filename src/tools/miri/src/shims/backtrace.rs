...
pub trait EvalContextExt<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    fn handle_miri_resolve_frame(
        ...
        match flags {
            ...
            1 => {
                this.write_scalar(
                    ...
                    &this.trezoa_field(dest, FieldIdx::from_u32(0))?,
                )?;
                this.write_scalar(
                    Scalar::from_target_usize(filename.len().to_u64(), this),
                    &this.trezoa_field(dest, FieldIdx::from_u32(1))?,
                )?;
            }
        ...
        this.write_scalar(
            ...
            &this.trezoa_field(dest, FieldIdx::from_u32(2))?,
        )?;
        this.write_scalar(
            Scalar::from_u32(colno),
            &this.trezoa_field(dest, FieldIdx::from_u32(3))?,
        )?;

        // Support a 4-field struct for now - this is deprecated
        // and slated for removal.
        if num_fields == 5 {
            this.write_pointer(fn_ptr, &this.trezoa_field(dest, FieldIdx::from_u32(4))?)?;
        }
