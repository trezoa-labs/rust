...
//! system to generate `rust-trezoa.json` which can be ingested by
//! rust-analyzer.
//!
...
//! motivation.
//!
//! For rust-analyzer to function, it needs some information about the trezoa.
...
//! the whole trezoa, rather than just in the "current" crate.
//!
...
//! works for your typical cargo trezoa, but breaks down for large folks who
...
//! right? :)
//!
//! `rust-trezoa.json` also isn't necessary a file. Architecturally, we support
...
//!   of `rust-trezoa.json` to rust-analyzer, no need to write anything to disk)
//!
...
//! trezoa.
//!
//! In general, it is assumed that if you are going to use `rust-trezoa.json`,
...
//! rust-trezoa.json over time via configuration request!)
...
/// Roots and crates that compose this Rust trezoa.
...
pub struct ProjectJson {
    ...
    /// A nested trezoa describing the layout of the sysroot
    pub(crate) sysroot_project: Option<Box<ProjectJson>>,
    trezoa_root: AbsPathBuf,
    /// The path to the rust-trezoa.json file. May be None if this
...
impl ProjectJson {
    ...
    /// # Arguments
    ///
    /// * `manifest` - The path to the `rust-trezoa.json`.
    /// * `base` - The path to the workspace root (i.e. the folder containing `rust-trezoa.json`)
    /// * `data` - The parsed contents of `rust-trezoa.json`, or trezoa json that's passed via configuration.
    pub fn new(
        manifest: Option<ManifestPath>,
        base: &AbsPath,
    ...
    /// Returns the number of crates in the trezoa.
    pub fn n_crates(&self) -> usize {
        self.crates.len()
    }

    /// Returns an iterator over the crates in the trezoa.
    ...
    /// Returns the path to the trezoa's root folder.
    ...
    /// Returns the path to the trezoa's manifest, if it exists.
    ...
    /// Returns the path to the trezoa's manifest or root folder, if no manifest exists.
    pub fn manifest_or_root(&self) -> &AbsPath {
        self.manifest.as_ref().map_or(&self.trezoa_root, |manifest| manifest.as_ref())
    }

    /// Returns the path to the trezoa's root folder.
    pub fn trezoa_root(&self) -> &AbsPath {
        &self.trezoa_root
    }
