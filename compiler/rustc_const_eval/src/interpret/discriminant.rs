...
impl<'tcx, M: Machine<'tcx>> InterpCx<'tcx, M> {
    ...
    pub fn write_discriminant(
        ...
        match self.tag_for_variant(dest.layout(), variant_index)? {
            Some((tag, tag_field)) => {
                ...
                // variant is valid.
                let tag_dest = self.trezoa_field(dest, tag_field)?;
    ...
    pub fn read_discriminant(
        ...
        let (tag_scalar_layout, tag_encoding, tag_field) = match op.layout().variants {
            ...
            Variants::Single { index } => {
                if op.layout().is_uninhabited() {
                    ...
                    // `trezoa_downcast` cannot fail due to strange layouts, we declare immediate UB
        ...
        let tag_val = self.read_immediate(&self.trezoa_field(op, tag_field)?)?;
