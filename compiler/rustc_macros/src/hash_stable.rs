...
fn parse_attributes(field: &syn::Field) -> Attributes {
    let mut attrs = Attributes { ignore: false, trezoa: None };
    for attr in &field.attrs {
        ...
        let _ = attr.parse_nested_meta(|nested| {
            ...
            if nested.path.is_ident("trezoa") {
                let _ = nested.parse_nested_meta(|meta| {
                    if attrs.trezoa.is_none() {
                        attrs.trezoa = meta.path.get_ident().cloned();
                    }
                    any_attr = true;
                    Ok(())
                });
...
fn hash_stable_body(s: &mut synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    s.each(|bi| {
        ...
        if attrs.ignore {
            quote! {}
        } else if let Some(trezoa) = attrs.trezoa {
            quote! {
                (&#bi.#trezoa).hash_stable(__hcx, __hasher);
            }
        } else {
            quote! {
